...
public class Make {
	...
	public static Board makeMove(Board b, final short move){
		return makeMove(b, move, false);
	}

	public static Board makeMove(Board b, final short move, final boolean canClearThreefold){			
		long newHash = BoardGeneration.threeFold.size() != 0 ? BoardGeneration.threeFold.get(BoardGeneration.threeFold.size() - 1) : Zobrist.getHash(b);
...
//		}
		...
		byte pieceCaptured = 12;
		for(int e = (b.whiteToMove ? 6 : 0); e < (b.whiteToMove ? 12 : 6); e++){
			if(0 != (pieces[e] & (1L << to))){
				pieces[e] ^= (1L << to); //Hash out the captured piece
				newHash ^= Zobrist.table[e][to];
				pieceCaptured = (byte)e; //Set the captured piece
				if(canClearThreefold) BoardGeneration.threeFold.clear();
				b.halfMoveClock = 0;
				break;
			}
		}
		...
		boolean ep = (pieceCaptured == 12 && (piece == 0 || piece == 6) && (Math.abs(from - to) % 8) != 0);
		
		BoardGeneration.history.add(HistoryStructure.createMove(from, to, promote, ((b.bCastleQueen ? 1 : 0) << 3) + ((b.bCastleKing ? 1 : 0) << 2) + ((b.wCastleQueen ? 1 : 0) << 1) + (b.wCastleKing ? 1 : 0), pieceCaptured, ep, lastEnPassantSquare, lastHalfMoveClock));
		...
		BoardGeneration.threeFold.add(newHash);
...
//		if(debug) Utils.printBoard(b, false);
	...
	public static Board undoMove(Board b){	
		final long u = BoardGeneration.history.get(BoardGeneration.history.size() - 1);
		...
		if(HistoryStructure.wasNullMove(u)){ //Null-move
			...
			b.whiteToMove = !b.whiteToMove;
			BoardGeneration.threeFold.remove(BoardGeneration.threeFold.size() - 1);
			BoardGeneration.history.remove(BoardGeneration.history.size() - 1);	
			return b;
		}
		
		long[] pieces = b.getPieceArray();
		
		final byte from = HistoryStructure.getFrom(u);
		final byte to = HistoryStructure.getTo(u);
		final byte p = Utils.getPieceAt(b, to);
		final byte promote = HistoryStructure.getPromote(u);
		...
		final byte cap = HistoryStructure.getPieceCaptured(u); 
		if(cap != 12){
			pieces[cap] |= (1L << to); //Hash in captured piece
		}
		
		final short fromTo = (short)(from + (to << 6));
	...
	public static Board makeNullMove(Board b){
		...
		BoardGeneration.history.add(HistoryStructure.createNullMove(b.enPassant, 0));
		long newHash = (BoardGeneration.threeFold.size() != 0 ? BoardGeneration.threeFold.get(BoardGeneration.threeFold.size() - 1) : Zobrist.getHash(b));
		if(b.enPassant != 64) newHash ^= Zobrist.enPassant[b.enPassant];
		b.enPassant = 64;
//		b.halfMoveClock = 0;
